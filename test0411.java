/*题目描述：
        ...
        对每组测试数据，在单独的行中输出小B的旅行方向。 forward - 由N到M方向； backward -由M到N方向 both - 两种方向都有可能； invalid - 不可能看到这样的颜色序列；
...
public class test0411 {
    public static void main(String[] args){
        ...
        while(in.hasNext()){
            ...
            int se = sequence(s1,s2,s3);
            int re = reserve(s1,s2,s3);
            if(se + re == 2){
    ...
    public static int sequence(String s1,String s2,String s3){
        int a = s1.indexOf(s2);
        int b = s1.lastIndexOf(s3);
        if( a != -1 && b != -1  ){
            if(a + s2.length()- 1 < b){
                return 1;
            }
    ...
    public static int reserve(String s1,String s2,String s3){
        int a = s1.lastIndexOf(s2);
        int b = s1.indexOf(s3);
        if( a != -1 && b != -1 ){
            if(b + s3.length() - 1 < a){
                return 1;
